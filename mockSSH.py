...
with open('id_rsa') as privateBlobFile:
    ...
    privateKeyString  = Key.fromString(data=privateBlob)

with open('id_rsa.pub') as publicBlobFile:
    publicBlob = publicBlobFile.read()
    publicKeyString  = Key.fromString(data=publicBlob)
...
def getRSAKeys(keypath=".", public_key = 'id_rsa.pub', private_key = 'id_rsa'):
    if not os.path.exists(keypath):
        print "Could not find specified keypath (%s)" % keypath
        sys.exit(1)

    pubkey = os.path.join(keypath, public_key)
    privkey = os.path.join(keypath, private_key)
